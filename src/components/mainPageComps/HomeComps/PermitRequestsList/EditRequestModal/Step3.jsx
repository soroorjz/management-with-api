...
import { MdDelete } from "react-icons/md"; // استفاده از MdDelete برای یکپارچگی
import { IoCloseSharp } from "react-icons/io5";
import { IoClose } from "react-icons/io5";
import { Tooltip } from "react-tooltip"; // وارد کردن react-tooltip
import "./Step3.scss";
...
const Step3 = ({
  ...
  isReadOnly = false,
  addEducationRow,
  handleDeleteEducationRow, // پراپ جدید برای حذف ردیف
}) => {
  ...
  const handleRemoveField = (fieldToRemove, rowIndex) => {
    ...
    const currentFields = Array.isArray(formData.educationTable[rowIndex].field)
                        ...
                        {fieldOptions.map((option) => (
                          <label key={option.value} className="checkbox-label">
                            <input
                              type="checkbox"
                              value={option.value}
                              checked={(row.field || []).includes(option.value)}
                              onChange={(e) =>
